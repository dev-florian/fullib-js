...
void main() {
    ...
    float realnoise = 0.5 * (cnoise(vec4(newUV.x * scaleX + 0.0 * time / 3.0, newUV.y * scaleY, 0.0 * time / 3.0, 0.0)) + 1.0);

    float w = width * dt;

    // Uniform transition without horizontal slide
    float maskvalue = smoothstep(1.0 - w, 1.0, progress);
